...
function startLoader() {
  const counter = document.querySelector(".loader__count .count__text");
  let t = 0;

  (function updateCounter() {
    if (t < 100) {
      // faster increments
      let increment = Math.floor(20 * Math.random()) + 5; 
      t = Math.min(t + increment, 100);
      counter.textContent = t;

      // shorter timeout for faster count
      let timeout = Math.floor(50 * Math.random()) + 20;
...
function hideLoader() {
  gsap.to(".loader__count", { duration: 0.5, ease: "power2.in", y: "100%" });
  gsap.to(".loader__wrapper", { duration: 0.5, ease: "power4.in", y: "-100%", onComplete: () => {
...
function pageAppearance() {
  ...
  gsap.to(fadeInItems, { duration: 0.6, ease: "none", opacity: 1 });
}
...
$(window).on("load", function () {
  if ($(".animated-type").length) {
    new Typed("#typed", {
      ...
      backSpeed: 20,  // faster backspace
      backDelay: 1500, // shorter delay
    });
  }
});
...
ScrollTrigger.matchMedia({
  ...
  // Mobile only (animations disabled)
  "(max-width: 767px)": function () {
    // Reset any inline transform or opacity so content appears normally
    document.querySelectorAll(".anim-uni-scale-in-right, .anim-uni-scale-in-left").forEach((e) => {
      gsap.set(e, { clearProps: "all" }); // removes GSAP styles
    });
  },
});
...
function enforceLightTheme() {
  ...
  // Update the button icon (optional)
  themeBtn.innerHTML = '<i class="ph-bold ph-sun-horizon"></i>';
}

// Make sure it stays light on load
window.addEventListener("DOMContentLoaded", enforceLightTheme);

// Also fix it immediately if someone tries to toggle
themeBtn.addEventListener("click", enforceLightTheme);
